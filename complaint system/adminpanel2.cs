...
namespace complaint_system
{
    public partial class adminpanel2 : UserControl
    {
        ...
        public void load()
        {
            SqlDataAdapter adp = new SqlDataAdapter("select * from SUIGAS", con.connect());
       ...
       // SqlConnection co;
        public void display()
        {
            SqlDataAdapter adp = new SqlDataAdapter("select * from SOLVED_SUIGAS", con.connect());
            SqlCommand cm = new SqlCommand("insert into SOLVED_SUIGAS(NAME,CNIC,AREA,SUB_AREA,COMPLAINT)values('" + textBox1.Text + "','" + textBox2.Text + "','" + textBox3.Text + "','" + textBox4.Text + "','" + textBox5.Text + "')", con.connect());
            cm.ExecuteNonQuery();
            DataTable d = new DataTable();
            adp.Fill(d);
            dataGridView2.DataSource = d;

            SqlCommand cmd = new SqlCommand("delete from SUIGAS where ID=" + textBox6.Text + "", con.connect());
            cmd.ExecuteNonQuery();
            load();
            textBox6.Clear();
            textBox1.Clear();
